...
#Load packages.............................................................................
MetaRareSap<- read.csv("Metadata/Sap/MetaRareSppSap-Soil.csv", na.strings = "NA", header = TRUE)
...
MetaRareSap$Plot<- as.factor(MetaRareSap$Plot)
MetaRareSap$TSFdays<-as.numeric(MetaRareSap$TSFdays)
MetaRareSap$Treatment<-as.factor(MetaRareSap$Treatment)
MetaRareSap$Treatment <- try(relevel(MetaRareSap$Treatment,"Unburned")) #change base level
MetaRareSap$AshSeverity<-ordered(MetaRareSap$AshSeverity,c("Unburned","Low","Moderate","High"))
MetaRareSap$Date<-ordered(MetaRareSap$Date, c("9/30/18","10/8/18","10/17/18","11/19/18",
...
shapiro.test(MetaRareSap$S.obs)#< 2.2e-16------Non-normal
...
Pois1 <- glm(S.obs ~ Treatment, data = MetaRareSap, family = poisson(link = log))
ods <- Pois1$deviance / Pois1$df.residual;ods#0verdispersion 0.91
par(mfrow=c(2,2))
...
ods2<- Pois2$deviance / Pois2$df.residual;ods2 #0.91

Pois3 <- glm(S.obs ~ Treatment,family = poisson(link = identity))
ods3<- Pois3$deviance / Pois3$df.residual;ods3##0.91
...
nb1 <- glm.nb(S.obs ~Treatment,data = MetaRareSap)
...
MetaRareSap$TSF<-scale(MetaRareSap$TSFdays);summary(MetaRareSap$TSF)
MetaRareSap$Precip<-scale(MetaRareSap$TotPrecip)

#Explore Temporal correlations to ensure that we are capturing as much of the variance 
ASVneg1 <- glmer.nb(S.obs ~ (1|Plot) + (1|Subplot)+(1|TSFdays),data = MetaRareSap);summary(ASVneg1)
ASVneg2 <- glmer.nb(S.obs ~ (1|Plot)+(1|TSFdays),data = MetaRareSap);summary(ASVneg2)
ASVneg3 <- glmer.nb(S.obs ~ (1|Subplot)+(1|TSFdays),data = MetaRareSap);summary(ASVneg3)


#MuMIN for model selection................................
AICc(ASVneg1,ASVneg2,ASVneg3)#ASVneg1 better 499
...
Full<-glmer.nb(S.obs~Treatment*TSF + Treatment*Precip + Treatment*InitialAshDepth + 
                 ...
                 (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Full)

Null<-glmer.nb(S.obs ~ (1|Plot) + (1|Subplot) + (1|TSFdays), data=MetaRareSap);summary(Null)
...
Mod1<-glmer.nb(S.obs~Treatment*TSF + Treatment*Precip + Treatment*InitialAshDepth + 
                 TSF*Precip + TSF*InitialAshDepth + Treatment + TSF + Precip + InitialAshDepth +
                 (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod1)
...
Mod2<-glmer.nb(S.obs ~ Treatment*TSF + Treatment*Precip + Treatment*InitialAshDepth + 
          ...
          (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod2)
...
Mod3<-glmer.nb(S.obs ~ Treatment*TSF + Treatment*Precip + Treatment*InitialAshDepth + 
              ...
              (1|TSFdays),data = MetaRareSap);summary(Mod3)
anova(Mod2,Mod3)#Mod 2 beter, Not significant, remove
...
Mod4<-glmer.nb(S.obs ~ Treatment*TSF + Treatment*Precip +   Treatment + TSF +
            ...
            data = MetaRareSap);summary(Mod4)
anova(Mod2,Mod4)# Mod 2 better, Not significant, remove


#Treatment*Precip +
Mod5<-glmer.nb(S.obs ~ Treatment*TSF + Treatment + TSF + Precip + InitialAshDepth + 
            (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod5)
anova(Mod2,Mod5)#Mod 2 better, not signif, remove


#Treatment*TSF +
Mod6<-glmer.nb(S.obs ~ Treatment + TSF + Precip + InitialAshDepth + (1|Plot) +
                  (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod6)
anova(Mod2,Mod6)#Mod 2 better, signif, do not remove...............................

#Treatment + 
Mod7<-glmer.nb(S.obs ~ TSF + Precip + InitialAshDepth + Treatment*TSF + (1|Plot) + 
              (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod7)
anova(Mod2,Mod7)#Mod 2 better, not signif, remove

# TSF +
Mod8<-glmer.nb(S.obs ~ Precip + InitialAshDepth + Treatment*TSF +
                 (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod8)
anova(Mod2,Mod8)#Mod 2 better, not signif, remove


#Precip + 
Mod9<-glmer.nb(S.obs ~ InitialAshDepth + Treatment*TSF + (1|Plot) + 
            (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(Mod9)
anova(Mod2,Mod9)#Mod 2 better, not signif, remove


#InitialAshDepth + 
Mod10<-glmer.nb(S.obs ~Treatment*TSF + (1|Plot) + (1|Subplot) +
                  (1|TSFdays),data = MetaRareSap);summary(Mod10)
anova(Mod2,Mod10)#Mod 10 better, not signif, remove
...
FullNB<-glmer.nb(S.obs ~Treatment*TSF + (1|Plot) + (1|Subplot) +
                   (1|TSFdays),data = MetaRareSap);summary(FullNB)
R2Full<-r.squaredGLMM(FullNB);R2Full#Delta:0.20908284 0.24881180
...
capture.output(summary(FullNB),file="1-Analysis/Diversity/Alpha/Guilds/Tables/Sap-NegBinom-FullModel.csv")
write.csv(R2Full,"1-Analysis/Diversity/Alpha/Guilds/Tables/Sap-NegBinom-R2FullModel.csv")
...
TrTSFNB<-glmer.nb(S.obs ~ Treatment * TSF2 + (1|Plot) + (1|Subplot) + (1|TSFdays),data = MetaRareSap);summary(TrTSFNB)
...
PrecipNB<-glmer.nb(S.obs ~ Treatment * Precip + TSF * Precip +(1|Plot)+(1|Subplot)+(1|TSFdays),data = MetaRareSap);summary(PrecipNB)
...
AshDepthNB<-glmer.nb(S.obs ~ Treatment * AshDepth + TSF * AshDepth +(1|Plot)+(1|Subplot)+(1|TSFdays),data = MetaRareSap);summary(AshDepthNB)
...
TrtTSF<-glmer.nb(S.obs ~ Treatment + TSF2 + (1|Plot) +  (1|Subplot) + (1|TSFdays),
                 data = MetaRareSap);summary(TrtTSF)
